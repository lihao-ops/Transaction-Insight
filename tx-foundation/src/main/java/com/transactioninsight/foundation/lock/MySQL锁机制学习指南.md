...
验证 InnoDB 在 **唯一索引 + 等值查询 + FOR UPDATE** 下，只会锁定**这一条记录本身**，不会锁定前后间隙，也不会影响其他记录。
...
| 线程  | 行为                       | 预期                      |
| --- | ------------------------ | ----------------------- |
| 线程1 | SELECT ACC001 FOR UPDATE | 获取记录锁                   |
| 线程2 | UPDATE ACC001            | 必须被阻塞                   |
| 线程3 | UPDATE ACC002            | 立即成功（因为 ACC002 不是同一条记录） |
...
[线程3] 尝试更新 ACC002...
[线程3] ✓ 更新 ACC002 成功（不受 ACC001 锁影响）
```
...
* **记录锁只锁住一条记录，不影响其他行**
* ACC001 被锁住后，ACC002 完全不受影响
* 只有针对同一条记录的写请求会等待

这就是 InnoDB 高并发能力强的核心原因。

---

---

# 🔥 场景9：临键锁（Next-Key Lock = Record + Gap）🔐

## 📌 目的

验证范围查询 + FOR UPDATE 时，InnoDB 会使用临键锁（next-key lock），不仅锁住记录，还会锁住间隙，从而阻止幻读。
...
```sql
SELECT * FROM account WHERE balance BETWEEN 500 AND 3000 FOR UPDATE;
```

→ 会锁住 `(500,3000]` 之间的所有记录与间隙
...
```
[线程1] 执行范围查询 FOR UPDATE（产生 next-key lock）
  → 锁定 (500,1000], (1000,2000], (2000,3000]

[线程2] 尝试插入 ACC999(balance=1000)
  → 插入被阻塞超过 3000ms（被 next-key lock 限制）
...
| 锁类型                   | 锁住内容      | 会阻塞 INSERT？  | 场景                   |
| --------------------- | --------- | ------------ | -------------------- |
| **记录锁** Record Lock   | 精确匹配的一条记录 | ❌ 不阻塞其它记录    | 唯一索引等值查询             |
| **间隙锁** Gap Lock      | 记录之间的空隙   | ✔️ 阻塞插入      | 范围查询                 |
| **临键锁** Next-Key Lock | 记录本身 + 间隙 | ✔️ 阻塞插入 + 修改 | RR 下 FOR UPDATE 范围查询 |
